...
interface DocRef {
    ...
    val cachedReadable: StateFlow<KRead?>

    suspend fun change(block: suspend ChangeBlock.() -> Unit)

    suspend fun sync(syncMessage: ByteArray)

    /** Establish connection to the referenced document. */
    suspend fun connect()

    /** Disconnect from the referenced document. */
    suspend fun disconnect()
}

interface ChangeBlock : KRead, KWrite

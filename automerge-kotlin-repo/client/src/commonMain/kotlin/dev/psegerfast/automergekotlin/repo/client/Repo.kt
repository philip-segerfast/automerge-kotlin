...
interface NetworkAdapter {
    ...
    /**
     * Fetch a document.proto from the network with the given ID. Null if there's no such document.proto.
     * */
...
interface StorageAdapter {
    /** Load the document.proto from storage with the given ID. */
    suspend fun load(documentId: String): DocHandle?
    /** Save the document.proto to storage with the given ID. */
...
class CachedDocHandle(private val document: KDocument) : DocHandle {
    ...
    val documentValue: KReadable = document as KReadable
...
/** DocHandle which is to a non-physical document.proto. */
...
interface DocHandle {
    ...
    /** Apply an incoming update to the document.proto. Return the current sync state of the document.proto or null if it's all up-to-date. */
